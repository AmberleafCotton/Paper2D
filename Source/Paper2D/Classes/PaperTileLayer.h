...
public:
	UE_API void ConvertToTileSetPerCell();
	...
	static UE_API FTransform GetTileTransform(int32 FlagIndex);

	// Returns the number of occupied cells in the layer
	UE_API int32 GetNumOccupiedCells() const;
	...
	// Should this layer generate collision?
	// Note: This only has an effect if the owning tile map has collision enabled
	bool GetLayerCollides() const { return bLayerCollides; }
...
protected:
	UE_API void ReallocateAndCopyMap();
