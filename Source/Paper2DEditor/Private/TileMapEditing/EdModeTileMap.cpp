...
bool FEdModeTileMap::InputKey(FEditorViewportClient* InViewportClient, FViewport* InViewport, FKey InKey, EInputEvent InEvent)
{
	...
	if (InViewportClient->EngineShowFlags.ModeWidgets)
	{
		...
		{
			// Stopping painting - commit or cancel based on paintbrush tool
			if (GetActiveTool() == ETileMapEditorTool::Paintbrush && bIsStrokeActive)
			{
				// Commit the preview stroke (left click released)
				CommitPreviewStroke();
			}
...
bool FEdModeTileMap::FloodFillTiles(const FViewportCursorLocation& Ray)
{
	...
	if (UPaperTileLayer* TargetLayer = GetSelectedLayerUnderCursor(Ray, /*out*/ DestTileX, /*out*/ DestTileY))
	{
		...
		{
			FHorizontalSpan Span = OutstandingSpans.Pop(EAllowShrinking::No);
...
//////////////////////////////////////////////////////////////////////////
// Preview Stroke System Functions
...
{
	// Get the tile we want to paint from the source layer
	UPaperTileLayer* SourceLayer = GetSourceInkLayer();
	...
	// Paint each tile in the brush
	for (int32 BrushY = 0; BrushY < BrushHeight; ++BrushY)
	{
		for (int32 BrushX = 0; BrushX < BrushWidth; ++BrushX)
		{
			const int32 TargetX = TilePos.X + BrushX;
			const int32 TargetY = TilePos.Y + BrushY;
			FIntPoint TargetTilePos(TargetX, TargetY);
			
			// Skip if out of bounds
			if (TargetX < 0 || TargetY < 0 || TargetX >= Layer->GetLayerWidth() || TargetY >= Layer->GetLayerHeight())
			{
				continue;
			}
			
			// Store original tile if not already stored (for cancel/undo)
			if (!OriginalStrokeTiles.Contains(TargetTilePos))
			{
				OriginalStrokeTiles.Add(TargetTilePos, Layer->GetCell(TargetX, TargetY));
			}
			
			// Get the tile info from source layer
			FPaperTileInfo TileInfo = SourceLayer->GetCell(BrushX, BrushY);
...
{
	// Bresenham's line algorithm to interpolate path between mouse positions
	TArray<FIntPoint> TilePath = BresenhamLine(Start, End);
	
	// Paint each tile along the path (skip duplicates to avoid redundant work)
...
void FEdModeTileMap::CommitPreviewStroke()
{
	...
	// Create the undo transaction for the entire stroke
	...
	// NOW paint all tiles at once (batched, not per-tile during drag!)
	FBox DirtyRect(ForceInitToZero);
	const int32 LayerIndex = PreviewStrokeLayer->GetLayerIndex();
	
	for (const TPair<FIntPoint, FPaperTileInfo>& Pair : PreviewStrokeTiles)
	{
		// Actually paint the tile now
		PreviewStrokeLayer->SetCell(Pair.Key.X, Pair.Key.Y, Pair.Value);
		DirtyRect += FVector(Pair.Key.X, Pair.Key.Y, LayerIndex);
	}
	
	// Single PostEditChange for entire stroke (not per tile!)
	PreviewStrokeLayer->GetTileMap()->PostEditChange();
	
	// Add to dirty regions for nav mesh update
	if (DirtyRect.IsValid)
	{
		new (PendingDirtyRegions) FTileMapDirtyRegion(FindSelectedComponent(), DirtyRect);
	}
