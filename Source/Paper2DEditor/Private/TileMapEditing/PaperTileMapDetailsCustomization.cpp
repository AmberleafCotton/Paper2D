...
void FPaperTileMapDetailsCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailLayout)
{
	...
	IDetailCategoryBuilder& TileMapCategory = DetailLayout.EditCategory("Tile Map", LOCTEXT("TileMapDetailCategory", "Tile Map"), ECategoryPriority::Important);
	...
	if (TSharedPtr<IDetailsView> DetailsView = DetailLayout.GetDetailsViewSharedPtr())
	{
		CommandList = DetailsView->GetHostCommandList();
	}
