...
#include "PaperTileLayer.h"  // Required for FPaperTileInfo in TMap
...
protected:
	...
	TArray<FIntPoint> BresenhamLine(const FIntPoint& Start, const FIntPoint& End);
	void PaintTileToPreview(const FIntPoint& TilePos, UPaperTileLayer* Layer);
	void PaintAlongLineToPreview(const FIntPoint& Start, const FIntPoint& End, UPaperTileLayer* Layer);
	void CommitPreviewStroke();
	void CancelPreviewStroke();
...
protected:
	...
	// Preview stroke system (for continuous painting with commit/cancel)
	FIntPoint LastPaintedTilePosition;  // Last tile position painted in current stroke
	bool bHasLastPaintedPosition;  // Whether we have a valid last position
	TMap<FIntPoint, FPaperTileInfo> PreviewStrokeTiles;  // Tiles painted in current stroke (before commit)
